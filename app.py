...
# ── API 调用 ──────────────────────────────────────────────────────────
def call_api(model: str, system_prompt: str, user_message: str) -> str:
...
# ── 问题生成 ──────────────────────────────────────────────────────────
def build_question_prompt(template: str, keyword: str, description: str, count: int) -> str:
    # 替换第一行的数量
    prompt = re.sub(r"\b\d+\b", str(count), template, count=1)
    # 替换关键词占位符
    prompt = prompt.replace("xxxx", keyword).replace("Nano Banana 2", keyword)
...
q_templates = {
    ...
    for f in sorted(glob.glob(os.path.join(BASE_DIR, "*question*.md")))
}
answer_prompts = {
...
if not q_templates:
    st.error("未找到问题生成模板文件（文件名需含 'question'）。")
    st.stop()
...
with col_left:
    q_template_name = st.selectbox("问题生成模板", list(q_templates.keys()))
with col_right:
    model = st.text_input("模型", value="gemini-2.5-flash")

st.markdown("**关键词列表**（点击 + 添加行，说明列可留空）")
kw_df = st.data_editor(
    pd.DataFrame({"关键词": [""], "说明（可选，用于消除歧义）": [""]}),
    num_rows="dynamic",
    use_container_width=True,
    column_config={
        "关键词": st.column_config.TextColumn(width="medium"),
        "说明（可选，用于消除歧义）": st.column_config.TextColumn(width="large"),
    },
)
...
if st.button("生成问题", type="primary", use_container_width=True):
    keywords = [
        (str(row["关键词"]).strip(), str(row["说明（可选，用于消除歧义）"]).strip())
        for _, row in kw_df.iterrows()
        if str(row["关键词"]).strip()
    ]
    if not keywords:
        st.warning("请至少输入一个关键词。")
        st.stop()

    q_template_text = open(q_templates[q_template_name], encoding="utf-8").read()
    all_questions: list[tuple[str, str]] = []  # (keyword, question)
    errors = []
    ...
    for i, (kw, desc) in enumerate(keywords):
        ...
        try:
            prompt = build_question_prompt(q_template_text, kw, desc, int(count_per_kw))
            raw = call_api(model, "", prompt)
            qs = parse_questions(raw, max_count=int(count_per_kw))
            for q in qs:
                all_questions.append((kw, q))
...
if "questions" in st.session_state and st.session_state["questions"]:
    ...
    with st.expander(f"查看问题列表（共 {len(qs)} 个）"):
        preview_df = pd.DataFrame(qs, columns=["关键词", "问题"])
    ...
    if st.button("生成回答", type="primary", use_container_width=True):
        ...
        total = len(questions) * len(selected_prompts)
        done = 0
        prog2 = st.progress(0)
        status2 = st.empty()
        ...
        for prompt_name in selected_prompts:
            system_prompt = open(answer_prompts[prompt_name], encoding="utf-8").read()
            rows = []
            for kw, question in questions:
                status2.text(f"[{prompt_name}] [{done % len(questions) + 1}/{len(questions)}] {question[:55]}...")
                try:
                    answer = call_api(model, system_prompt, question)
